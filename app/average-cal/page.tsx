...
const formatNumber = (
  ...
  if (fixed.includes(".")) {
    ...
    if (floatVal.toString() === fixed && precision === 0)
      return floatVal.toString(); // e.g. 10.0 with precision 0 should be 10
...
export default function AverageCalculatorVisualizer() {
  ...
  const currentMaxValue =
    numbersForCalculation.length > 0
      ? Math.max(...numbersForCalculation, 0)
      : 0;
  let yAxisMax = 10;
  if (currentMaxValue > 0) {
    const niceNumberMagnitude = Math.pow(
      10,
      Math.floor(Math.log10(currentMaxValue))
    );
    yAxisMax =
      Math.ceil(currentMaxValue / niceNumberMagnitude) * niceNumberMagnitude;
    if (currentMaxValue > yAxisMax * 0.95) {
      // If max value is too close to the top, increase yAxisMax
      yAxisMax =
        Math.ceil((currentMaxValue * 1.1) / niceNumberMagnitude) *
        niceNumberMagnitude;
    }
    if (yAxisMax === 0 && currentMaxValue > 0) yAxisMax = currentMaxValue * 1.2; // Handle edge if currentMax is small
    if (yAxisMax < 5 && currentMaxValue > 0)
      yAxisMax = 5; // Ensure a minimum sensible yAxisMax
    else if (yAxisMax < 10 && currentMaxValue > 5) yAxisMax = 10;
  }
  if (
    yAxisMax === 0 &&
    numbersForCalculation.length > 0 &&
    numbersForCalculation.every((n) => n === 0)
  ) {
        ...
        {showResults && average !== null && (
          <div className="mt-8 pt-6 border-t-2 border-blue-200">
            ...
            <div className="text-center mb-6">
              {" "}
              {/* Centered container for title and result */}
              <h2 className="text-xl sm:text-2xl font-semibold text-blue-700 inline">
                {" "}
                {/* Title inline */}
                <FaCalculator className="inline mr-2 mb-1" /> 计算结果：
              </h2>
              <span className="text-3xl sm:text-4xl font-bold text-blue-600 ml-2">
                {" "}
                {/* Result inline and larger */}
            ...
            <div className="mt-8 bg-gray-50 p-4 rounded-lg overflow-x-auto">
              ...
              <div
                className="flex"
                style={{
                  height: `${MAX_BAR_HEIGHT + X_AXIS_LABEL_AREA_HEIGHT}px`,
                }}
              >
                {/* Y-axis Area */}
                <div
                ...
                >
                  {yAxisTicks
                    ...
                    .map(
                      (
                        tick // Render from top to bottom (value-wise)
                      ) => (
                        <div
                          key={`tick-${tick.value}`}
                ...
                <div className="flex-grow flex flex-col">
                  ...
                  >
                    {numbersForCalculation.map((num, index) => {
                      const barHeight =
                        yAxisMax > 0 ? (num / yAxisMax) * MAX_BAR_HEIGHT : 0;
                      return (
                        <div
                          key={index}
                      ...
                      yAxisMax > 0 && (
                        <>
                          <div
                            ...
                            style={{
                              bottom: `${
                                (average / yAxisMax) * MAX_BAR_HEIGHT
                              }px`,
                              zIndex: 10,
                            }}
                          ></div>
                          <span
                            ...
                            style={{
                              bottom: `${Math.max(
                                (average / yAxisMax) * MAX_BAR_HEIGHT - 6,
                                -2
                              )}px`,
                  ...
                  <div
                    ...
                    style={{
                      height: `${X_AXIS_LABEL_AREA_HEIGHT}px`,
                      paddingTop: "4px",
                    }}
                  >
                    {numbersForCalculation.map((num, index) => (
            ...
            <div className="mt-2 text-center text-blue-700">
              总和: {formatNumber(sum)}, 数据数量: {count}
            </div>
