...
import { useState } from "react"; // FormEvent removed as buttons now directly call handleSubmit
...
export default function ShapeCalculator() {
  ...
  const handleSubmit = (calcType: "area" | "perimeter") => {
    ...
    if (shapeType === "rectangle") {
      ...
      if (isNaN(L_val) || L_val <= 0 || isNaN(W_val) || W_val <= 0) {
        ...
        if (calcType === "area") {
          result = L_val * W_val;
          currentFormula = `面积 = 长 × 宽 = ${formatNumber(
            L_val
          )} × ${formatNumber(W_val)}`;
        } else {
          result = 2 * (L_val + W_val);
          currentFormula = `周长 = 2 × (长 + 宽) = 2 × (${formatNumber(
            L_val
          )} + ${formatNumber(W_val)})`;
        }
      ...
      if (isNaN(S_val) || S_val <= 0) {
        ...
        L_val = S_val;
        W_val = S_val; // For unified display logic
        if (calcType === "area") {
          result = S_val * S_val;
          currentFormula = `面积 = 边长 × 边长 = ${formatNumber(
            S_val
          )} × ${formatNumber(S_val)}`;
        } else {
          result = 4 * S_val;
  ...
  return (
    <div className="relative bg-white max-w-5xl w-full p-6 sm:p-8 rounded-xl shadow-xl">
      {" "}
      {/* Max width increased for side-by-side */}
      ...
      <div className="flex flex-col md:flex-row md:gap-8">
        ...
        <div className="md:w-2/5 space-y-6 mb-8 md:mb-0">
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    value={length}
                    onChange={(e) =>
                      handleInputChange(e.target.value, setLength)
                    }
                    placeholder="例如: 10"
                ...
                <div>
                  ...
                  <input
                    ...
                    value={width}
                    onChange={(e) =>
                      handleInputChange(e.target.value, setWidth)
                    }
                    placeholder="例如: 5"
                    className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  />
            ...
            {shapeType === "square" && (
              <div>
                ...
                <input
                  ...
                  inputMode="decimal"
                  value={side}
                  onChange={(e) => handleInputChange(e.target.value, setSide)}
                  placeholder="例如: 7"
                  className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 pt-2">
            ...
            <button
              onClick={() => handleSubmit("perimeter")}
              disabled={!isFormValid()}
        ...
        <div className="md:w-3/5 md:border-l md:pl-8 border-gray-200">
          {" "}
          {/* Added border and padding for large screens */}
          {error && (
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                {calculationType === "area" && (
                  <span className="text-xl text-indigo-600"> (平方单位)</span>
                )}
                {calculationType === "perimeter" && (
                  <span className="text-xl text-indigo-600"> (单位)</span>
                )}
              </div>
                ...
                {displayWidth > 0 && displayHeight > 0 && (
                  <div
                    className={`relative border-2 transition-all duration-500 ease-out
                        ${
                          calculationType === "area"
                            ? "bg-indigo-100 border-indigo-500 animate-pulse-slow"
                            : "border-purple-500"
                        }
                        ${
                          calculationType === "perimeter"
                            ? "border-4 animate-pulse-perimeter"
                            : ""
                        }
                      `}
                    style={{
                  ...
                  >
                    <span className="absolute -top-6 left-1/2 transform -translate-x-1/2 text-xs text-gray-700 bg-white px-1 rounded">
                      {shapeType === "rectangle"
                        ? `长: ${formatNumber(parseFloat(length))}`
                        : `边: ${formatNumber(parseFloat(side))}`}
                    </span>
                    {shapeType === "rectangle" && (
