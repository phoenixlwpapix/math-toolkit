...
export default function ProblemCard({
  ...
  const handleChange = (key: string, value: string) => {
    ...
    if (value === "" || /^-?\d*\.?\d*$/.test(value)) {
      ...
      setError(
        `"${
          inputFields.find((f) => f.key === key)?.label || key
        }" 字段请输入有效数字。`
      );
    }
  ...
  const allInputsFilled = inputFields.every(
    (field) =>
      ...
      !isNaN(parseFloat(inputs[field.key]))
  );
  ...
  return (
    <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl p-6 sm:p-8 min-w-[320px] sm:min-w-[450px] md:min-w-[500px]">
      ...
      <h1 className="text-2xl font-bold text-center text-blue-800 mt-8 mb-4">
        {title}
      </h1>{" "}
      {/* Added mt-8 to give space for absolute positioned link */}
      {description && (
      ...
      <div className="space-y-4">
        ...
        {result &&
          !error && ( // Only show result if no error
